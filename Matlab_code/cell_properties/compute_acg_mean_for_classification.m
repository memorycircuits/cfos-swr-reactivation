function result = compute_acg_mean_for_classification(custom_settings)
% Compute acg_mean from open-field spike autocorrelograms.
...
bin_size_seconds = double(get_option_value(custom_settings, 'binSizeSeconds', 0.0005));
lag_duration_seconds = double(get_option_value(custom_settings, 'lagDurationSeconds', 0.05));
save_updated_all_cells = logical(get_option_value(custom_settings, 'saveUpdatedAllCells', true));
update_script_name = 'compute_acg_mean_for_classification.m';
...
session_summaries = repmat(empty_acg_summary(), numel(session_indices), 1);
...
for idx = 1:numel(session_indices)
    ...
    if isempty(open_field_folders)
        session_summaries(idx).status = 'skipped_no_open_field_folders';
        warning('Session %d: no open-field folders found.', session_index);
        continue
    end

    all_open_field_spikes = cell(numel(tt_files), 1);
    ...
    for of_idx = 1:numel(open_field_folders)
        ...
        for cell_idx = 1:min(numel(tt_files), numel(spike_times))
            all_open_field_spikes{cell_idx} = [all_open_field_spikes{cell_idx}; spike_times{cell_idx}(:)]; %#ok<AGROW>
        end
        processed_subsessions{end + 1} = folder_name; %#ok<AGROW>
    end

    if isempty(processed_subsessions)
        session_summaries(idx).status = 'skipped_no_readable_open_field_spikes';
        warning('Session %d: no readable open-field spike sessions.', session_index);
        continue
    end

    acg_mean = NaN(numel(tt_files), 1);
    for cell_idx = 1:numel(tt_files)
        spike_times = sort(double(all_open_field_spikes{cell_idx}(:)));
        spike_times = spike_times(isfinite(spike_times));
        if numel(spike_times) < 2
            continue
        end

        [narrow_acg, ~] = CrossCorrel(spike_times, spike_times, ...
            bin_size_seconds, lag_duration_seconds);
        acg_mean(cell_idx) = compute_positive_lag_acg_mean(narrow_acg, bin_size_seconds);
    end

    All_Cells_combined(session_index).acg_mean = acg_mean(:);
    last_updated = mark_fields_updated(last_updated, session_index, ...
        {'acg_mean'}, update_script_name);

    session_summaries(idx).status = 'processed';
    session_summaries(idx).cell_count = numel(acg_mean);
    session_summaries(idx).finite_acg_mean_count = nnz(isfinite(acg_mean));
...
result = struct( ...
    ...
    'binSizeSeconds', bin_size_seconds, ...
    'lagDurationSeconds', lag_duration_seconds, ...
    'sessions', session_summaries);

fprintf('Computed acg_mean for %d sessions.\n', ...
    nnz(strcmp({session_summaries.status}, 'processed')));
end

function acg_mean_ms = compute_positive_lag_acg_mean(narrow_acg, bin_size_seconds)

narrow_acg = double(narrow_acg(:));
acg_mean_ms = NaN;
if isempty(narrow_acg)
    return
end

offset = floor(numel(narrow_acg) / 2) + 1;
zero_window = max(1, offset - 1):min(numel(narrow_acg), offset + 1);
narrow_acg(zero_window) = 0;

positive_indices = (offset + 2):numel(narrow_acg);
if isempty(positive_indices)
    return
end

positive_counts = narrow_acg(positive_indices);
valid_mask = isfinite(positive_counts) & positive_counts > 0;
if ~any(valid_mask)
    return
end

positive_indices = positive_indices(valid_mask);
positive_counts = positive_counts(valid_mask);
positive_lags_ms = (positive_indices(:) - offset) * bin_size_seconds * 1000;
acg_mean_ms = sum(positive_lags_ms .* positive_counts(:)) / sum(positive_counts);
end
...
function summary = empty_acg_summary()

summary = struct( ...
    'session_index', NaN, ...
    'status', '', ...
    'cell_count', 0, ...
    'finite_acg_mean_count', 0, ...
    'processed_subsessions', {{}} ...
    );
end
